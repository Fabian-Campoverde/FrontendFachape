...
import { FaDrawPolygon, FaRulerHorizontal, FaVectorSquare,FaDownload } from 'react-icons/fa';
...
const MeasurementModal: React.FC<Props> = ({ isOpen, onClose, imageUrl, scale }) => {
  ...
  const handleStageClick = (e: any) => {
    ...
    if (tool === 'polygon') {
      ...
      if (isClosing) {
        setShapes([...shapes, { type: 'polygon', points: newPoints }]);
        setCurrent([]);
      } else {
        setCurrent(newPoints);
      }
  ...
  return (
    <React.Fragment key={i}>
      <Rect x={x} y={y} width={w} height={h} stroke="#2563EB" strokeWidth={2} />
      {/* Base (abajo) */}
      <KonvaText
...
>
  Polígono
          ...
          <Button variant="outlined" onClick={() => removeByType('line')}>X Líneas</Button>
          <Button variant="outlined" onClick={() => removeByType('rect')}>X Rect</Button>
          <Button variant="outlined" onClick={() => removeByType('polygon')}>X Pol</Button>
          <Button variant="outlined" onClick={() => setZoom(z => Math.min(z + 0.2, 5))}>Zoom +</Button>
          <Button variant="outlined" onClick={() => setZoom(z => Math.max(z - 0.2, 0.2))}>Zoom -</Button>
          <Button variant="contained" color="success" onClick={handleDownload}><FaDownload className="w-5 h-5 " /></Button>
        </div>
        <div className="overflow-auto  w-full flex justify-center" >
          ...
          >
            <Layer>{image && <KonvaImage image={image} width={image.width * zoom} height={image.height * zoom} />}</Layer>
            <Layer>
